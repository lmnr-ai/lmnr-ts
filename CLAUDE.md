...
- Only MODEL_STEP maps to Laminar `LLM` span type (per commit `2d9b868 cut out chunk spans`). MODEL_GENERATION and MODEL_CHUNK fall through to DEFAULT, but MODEL_GENERATION still carries gen_ai.* usage/model attributes. MODEL_CHUNK spans are excluded at the Mastra config layer via `excludeSpanTypes: ["model_chunk"]`.
