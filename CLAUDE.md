...
- **`MastraExporter.shutdown()` MUST drain in-flight `exportTracingEvent` promises before stopping the processor.** Mastra's `ObservabilityBus.emit()` calls each exporter's `exportTracingEvent` fire-and-forget (it tracks the promise on the bus, not the exporter). Users who call `mastra.shutdown()` directly bypass `observability.shutdown()`, which is the only path that flushes the bus first — so the final `span_ended` for the last root span can race past `processor.shutdown()` and `BatchSpanProcessor.onEnd` silently drops it (guarded by `_shutdownOnce.isCalled`). The exporter tracks `pendingEvents: Set<Promise<void>>` and awaits them in both `flush()` and `shutdown()` before touching the processor.
