...
describe("parseProjectFromMetadata", () => {
  it("extracts the projectId from the metadata JSON", () => {
    expect(parseProjectFromMetadata(JSON.stringify({ projectId: UUID }))).toBe(
      UUID,
    );
  });
  ...
  it("ignores unrelated keys", () => {
    expect(
      parseProjectFromMetadata(JSON.stringify({ projectId: UUID, foo: "bar" })),
    ).toBe(UUID);
  });

  it("returns null for a non-uuid projectId", () => {
    expect(
      parseProjectFromMetadata(JSON.stringify({ projectId: "not-a-uuid" })),
    ).toBeNull();
  });
