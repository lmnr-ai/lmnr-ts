...
const { mockBuildLaminarClient, mockResolveUserToken, mockLaminarClientCtor } =
  vi.hoisted(() => ({
...
describe("withProjectClient", () => {
  ...
  it("renders a JSON error and exits with the mapped code on failure", async () => {
    ...
    await expect(
      withProjectClient(action, exitCodeFor)({}, cmd),
  ...
  it("exits 1 by default in human mode on failure", async () => {
    ...
    await expect(withProjectClient(action)({}, cmd)).rejects.toThrow(
      "process.exit(1)",
    );
...
describe("withUserToken", () => {
  ...
  it("renders a JSON error and exits on failure", async () => {
    ...
    await expect(withUserToken(action)({}, cmd)).rejects.toThrow(
      "process.exit(1)",
    );
