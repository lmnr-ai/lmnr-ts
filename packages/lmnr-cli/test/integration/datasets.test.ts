...
describe("datasets CLI integration — with mock server", () => {
  ...
  beforeAll(async () => {
    mockServer = http.createServer((req, res) => {
      ...
      req.on("end", () => {
        ...
        if (
          req.method === "GET" &&
          req.url?.startsWith("/v1/cli/datasets/datapoints")
        ) {
          res.writeHead(200);
          res.end(
            JSON.stringify({
              items: mockDatapoints,
              totalCount: mockDatapoints.length,
            }),
          );
        } else if (
          req.method === "GET" &&
          req.url?.startsWith("/v1/cli/datasets")
        ) {
          res.writeHead(200);
  ...
  it("datasets push --json returns success with count", async () => {
    ...
    await fs.writeFile(
      dataFile,
      JSON.stringify([{ data: { x: 1 } }, { data: { x: 2 } }]),
    );
