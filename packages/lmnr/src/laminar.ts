...
export class Laminar {
  ...
  private static _initializeContextFromEnv(): void {
    ...
    try {
      ...
      if (laminarContext.spanPath && laminarContext.spanPath.length > 0) {
        baseContext = baseContext.setValue(
          CONTEXT_SPAN_PATH_KEY,
          laminarContext.spanPath,
        );
      }
      if (laminarContext.spanIdsPath && laminarContext.spanIdsPath.length > 0) {
        baseContext = baseContext.setValue(
          CONTEXT_SPAN_IDS_PATH_KEY,
  ...
  private static _startSpan({
    ...
    if (parentSpanContext) {
      try {
        ...
        logger.warn(
          "Failed to parse parent span context: " +
            (e instanceof Error ? e.message : String(e)),
        );
      }
    }
