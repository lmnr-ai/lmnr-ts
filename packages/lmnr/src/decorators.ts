...
>(options: ObserveOptions, fn: F, ...args: A): ReturnType<F> {
  if (fn === undefined || typeof fn !== "function") {
    throw new Error(
      "Invalid `observe` usage. Second argument `fn` must be a function.",
    );
  }
 ...
 * ```
 */
export function withTracingLevel<
  A extends unknown[],
  F extends (...args: A) => ReturnType<F>,
>(tracingLevel: TracingLevel, fn: F, ...args: A): ReturnType<F> {
  ...
  const newAssociationProperties = (entityContext.getValue(
    ASSOCIATION_PROPERTIES_KEY,
  ) ?? {}) as Record<string, any>;
  delete newAssociationProperties["tracing_level"];

  entityContext.setValue(ASSOCIATION_PROPERTIES_KEY, newAssociationProperties);
...
const buildContextProperties = (
  ...
  const sessionIdValue =
    options.sessionId ?? parentSessionId ?? ctxAssociationProperties.sessionId;
  if (sessionIdValue) {
    contextProperties.sessionId = sessionIdValue;
  }

  const userIdValue =
    options.userId ?? parentUserId ?? ctxAssociationProperties.userId;
  if (userIdValue) {
  ...
  const traceTypeValue =
    options.traceType ??
    parentTraceType ??
    (["EVALUATION", "EXECUTOR", "EVALUATOR"].includes(
      options.spanType ?? "DEFAULT",
    )
      ? "EVALUATION"
      : undefined) ??
...
const buildSpanAttributes = (
  ...
  if (options.tags) {
    spanAttributes[`${ASSOCIATION_PROPERTIES}.tags`] = Array.from(
  ...
  if (options.parentSpanContext) {
    try {
      const laminarContext =
        typeof options.parentSpanContext === "string"
      ...
      logger.warn(
        "Failed to parse parent span context for span attributes: " +
          errorMessage(e),
      );
    }
  }
