...
export const uuidToOtelTraceId = (uuid: string): string =>
  uuid.replace(/-/g, "");
export const uuidToOtelSpanId = (uuid: string): string =>
  uuid.replace(/-/g, "").slice(16);
...
export const tryToOtelSpanContext = (
  ...
  if (typeof spanContext === "string") {
    try {
      ...
      throw new Error(
        `Failed to parse span context ${spanContext}. ` +
    ...
    logger.warn(
      "The span context is already an OpenTelemetry SpanContext. " +
        "Returning it as is. Please use `LaminarSpanContext` objects instead.",
    );
    return spanContext;
  } else {
    throw new Error(
      `Invalid span context ${JSON.stringify(spanContext)}. ` +
        "Must be a LaminarSpanContext or its json representation.",
    );
  }
};

const recordToOtelSpanContext = (
  record: Record<string, unknown>,
): SpanContext => {
  if (
    (typeof record.spanId === "string" && typeof record.traceId === "string") ||
    (typeof record.span_id === "string" && typeof record.trace_id === "string")
  ) {
    return {
      spanId: uuidToOtelSpanId(
        (record?.spanId as string) ?? (record?.["span_id"] as string),
      ),
      traceId: uuidToOtelTraceId(
        (record?.traceId as string) ?? (record?.["trace_id"] as string),
      ),
    ...
    throw new Error(
      `Invalid span context ${JSON.stringify(record)}. ` +
...
export const deserializeLaminarSpanContext = (
  ...
  if (typeof traceId !== "string" || typeof spanId !== "string") {
    throw new Error(
      "Invalid LaminarSpanContext: traceId and spanId must be strings",
    );
  }
  ...
  if (!isStringUUID(traceId) || !isStringUUID(spanId)) {
    throw new Error(
      "Invalid LaminarSpanContext: traceId and spanId must be valid UUIDs",
    );
  }
  ...
  return {
    ...
    spanIdsPath: Array.isArray(spanIdsPath)
      ? (spanIdsPath as StringUUID[])
      : undefined,
...
export const isOtelAttributeValueType = (
  value: unknown,
  ...
  Object.fromEntries(
    Object.entries(metadata).map(([key, value]) => {
      if (isOtelAttributeValueType(value)) {
        ...
        return [
          `${ASSOCIATION_PROPERTIES}.metadata.${key}`,
