...
export class PlaywrightInstrumentation extends InstrumentationBase {
  ...
  constructor(
    ...
    super("@lmnr/playwright-instrumentation", SDK_VERSION, {
      enabled: true,
    });
    this._client = client;
  ...
  public async patchPage(page: Page, sessionId: StringUUID) {
    ...
    if (parentSpan) {
      return await Laminar.withSpan(parentSpan, wrapped);
    }
