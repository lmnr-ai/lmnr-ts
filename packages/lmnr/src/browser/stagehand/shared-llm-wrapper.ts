      ...
      return await laminarObserve(
        ...
        async () => {
          ...
          span.setAttributes({
            ...
            innerOptions.maxOutputTokens !== undefined
              ? {
                  "gen_ai.request.max_tokens":
                    innerOptions.maxTokens ?? innerOptions.maxOutputTokens,
                }
              : {}),
          ...
          innerOptions.messages?.forEach((message, index) => {
            span.setAttributes({
              ...
              [`gen_ai.prompt.${index}.content`]: JSON.stringify(
                message.content,
              ),
            });
          });
          ...
          innerOptions.tools?.forEach((tool, index) => {
            span.setAttributes({
              ...
              [`llm.request.functions.${index}.parameters`]: JSON.stringify(
                tool.parameters,
              ),
            });
          });
          ...
          const result = (await original
            .bind(this)
            .apply(this, args)) as LLMResponse;
          ...
          result.choices?.forEach((choice) => {
            ...
            choice.message.tool_calls?.forEach((toolCall, toolCallIndex) => {
              span.setAttributes({
                [`gen_ai.completion.${index}.message.tool_calls.${toolCallIndex}.id`]:
                  toolCall.id,
