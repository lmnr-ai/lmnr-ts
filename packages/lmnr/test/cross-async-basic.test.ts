...
void describe("Cross-Async Span Management - Basic Tests", () => {
  ...
  void it("start span in one async context and end it in another async context", async () => {
    ...
    assert.notStrictEqual(
      spanAResult!.spanContext().traceId,
      spanBResult!.spanContext().traceId,
    );
  });

  void it("start span in one async context and end it in another async context reverse", async () => {
    // Simulate a complex workflow with multiple async operations
  ...
  void it("startActiveSpan({ global: true }) parents spans created in unrelated async tasks", async () => {
    ...
    const outer = Laminar.startActiveSpan({
      name: "outer-global",
      global: true,
    });
    ...
    assert.strictEqual(
      getParentSpanId(childResult),
      outerResult.spanContext().spanId,
    );
  });

  void it("global span does not leak: spans created after it ends have no parent", async () => {
    const outer = Laminar.startActiveSpan({
      name: "outer-expired",
      global: true,
    });
    ...
    const detached = await new Promise<Span>((resolve) => {
      setImmediate(() =>
        resolve(Laminar.startActiveSpan({ name: "detached" })),
      );
    });
    detached.end();
