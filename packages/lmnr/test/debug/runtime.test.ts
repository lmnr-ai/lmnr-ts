...
void describe("DebugRuntime", () => {
  ...
  void it("updateContextConfig changes the same session", () => {
    const runtime = new DebugRuntime(
  ...
  void it("initDebugRuntimeFromContext refreshes a context runtime, reusing client", () => {
    ...
    assert.strictEqual(first.configChanged, true);
    ...
    assert.strictEqual(second.configChanged, true);
  ...
  void it("initDebugRuntimeFromContext never overrides an env-origin runtime", () => {
    ...
    assert.strictEqual(ctx.configChanged, false);
  ...
  void it("replayConfigured reflects both ids being present", () => {
    ...
    assert.strictEqual(
      new DebugRuntime(config(), fake, null).replayConfigured,
      true,
    );
    assert.strictEqual(
      new DebugRuntime(config({ replayTraceId: null }), fake, null)
        .replayConfigured,
      false,
    );
    assert.strictEqual(
      new DebugRuntime(config({ cacheUntilSpanId: null }), fake, null)
        .replayConfigured,
      false,
    );
  });
