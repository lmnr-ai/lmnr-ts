...
void describe("Stagehand v3 instrumentation — wrap guards", () => {
  ...
  void it("does not warn when wrapping a Stagehand instance with missing handlers", async () => {
    ...
    try {
      ...
      class StubStagehand {
        ...
        async act(_arg?: unknown) {
          return;
        }
        async extract(_arg?: unknown) {
          return;
        }
        async observe(_arg?: unknown) {
          return;
        }
      }
      ...
      const nonExistentWarnings = warnings.filter((w) =>
        /non[- ]existent method|not found|cannot wrap/i.test(w),
      );
      assert.deepEqual(
        nonExistentWarnings,
        [],
  ...
  void it("wraps apiClient methods when present (BROWSERBASE remote mode)", async () => {
    ...
    try {
      ...
      class StubStagehand {
        ...
        async act() {
          return this.apiClient!.act();
        }
        async extract() {
          return this.apiClient!.extract();
        }
        async observe() {
          return this.apiClient!.observe();
        }
      }
