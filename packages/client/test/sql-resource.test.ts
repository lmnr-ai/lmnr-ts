...
import { SqlResource } from "../src/resources/sql";
...
void describe("SQL Resource Tests", () => {
  void it("should send POST request to correct endpoint", async () => {
    ...
    const sql = new SqlResource("https://api.test.com:443", {
      type: "apiKey",
      key: "test-api-key",
    });
    ...
    assert.strictEqual(
      ...
      "https://api.test.com:443/v1/sql/query",
    );
  ...
  void it("should handle error responses", async () => {
    ...
    await assert.rejects(() => sql.query("INVALID QUERY"), /400/);
  });
});
