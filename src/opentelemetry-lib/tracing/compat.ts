...
import { ReadableSpan as OTelV2ReadableSpan } from "@opentelemetry/sdk-trace-base-v2";
import { Span as OTelV2Span } from "@opentelemetry/sdk-trace-base-v2";
...
export const makeSpanOtelV2Compatible = (
  ...
  if ((span as unknown as OTelV2Span).instrumentationScope && !(span as unknown as SdkSpan).instrumentationLibrary) {
    // Making the spans from V2 compatible with V1
    Object.assign(span, {
      instrumentationLibrary: (span as unknown as OTelV2Span).instrumentationScope,
    });
  } else if ((span as unknown as SdkSpan).instrumentationLibrary && !(span as unknown as OTelV2Span).instrumentationScope) {
...
export const getParentSpanId = (
  ...
  return (span as unknown as OTelV2Span).parentSpanContext?.spanId ?? (span as unknown as SdkSpan).parentSpanId;
}
