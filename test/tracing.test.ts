...
} from "../src/opentelemetry-lib/tracing/event-context";
...
void describe("tracing", () => {
  ...
  void it("cleans up old event contexts when LRU cache limit is exceeded", async () => {
    // This test verifies the LRU cache behavior by:
    // 1. Creating 1001 traces (exceeding the 1000 limit)
    // 2. Checking that the system still works correctly after cleanup
    // 3. Verifying that new contexts can still be set

    const results: Array<{ sessionId?: string, userId?: string }> = [];

    // Create a function that sets and immediately retrieves context
    const fnWithContext = (traceNum: number) => {
      setLaminarEventContext({
        sessionId: `session-${traceNum}`,
        userId: `user-${traceNum}`,
      });

      // Immediately test that context was set correctly within the same span
      const attributes = getEventAttributesFromCurrentContext();
      results.push({
        sessionId: attributes['lmnr.event.session_id'],
        userId: attributes['lmnr.event.user_id'],
      });

      return traceNum;
    };

    // Create 1001 traces to exceed the LRU limit of 1000
    // This will trigger the cleanup logic in our LRU cache
    const promises = [];
    for (let i = 1; i <= 1001; i++) {
      promises.push(observe({ name: `trace-${i}` }, fnWithContext, i));
    }
    await Promise.all(promises);

    // Verify we created 1001 spans
    const allSpans = exporter.getFinishedSpans();
    assert.strictEqual(allSpans.length, 1001, "Should have created 1001 spans total");

    // All traces should have successfully set their context during execution
    // (before any cleanup could affect them)
    assert.strictEqual(results.length, 1001, "Should have 1001 context results");

    for (let i = 0; i < results.length; i++) {
      const result = results[i];
      assert.strictEqual(result.sessionId, `session-${i + 1}`,
        `Trace ${i + 1} should have correct session ID during execution`);
      assert.strictEqual(result.userId, `user-${i + 1}`,
        `Trace ${i + 1} should have correct user ID during execution`);
    }

    // Test that the system still works correctly after LRU cleanup
    // by creating a few more traces
    const postCleanupResults: Array<{ sessionId?: string, userId?: string }> = [];

    for (let i = 1; i <= 5; i++) {
      const result = await observe({ name: `post-cleanup-${i}` }, () => {
        setLaminarEventContext({
          sessionId: `post-cleanup-session-${i}`,
          userId: `post-cleanup-user-${i}`,
        });
      ...
      postCleanupResults.push(result);
    }

    // Verify that new contexts work correctly after cleanup
    for (let i = 0; i < postCleanupResults.length; i++) {
      const result = postCleanupResults[i];
      assert.strictEqual(result.sessionId, `post-cleanup-session-${i + 1}`,
        `Post-cleanup trace ${i + 1} should have correct session ID`);
      assert.strictEqual(result.userId, `post-cleanup-user-${i + 1}`,
        `Post-cleanup trace ${i + 1} should have correct user ID`);
    }

    // Final verification: we should have 1006 total spans (1001 + 5)
    const finalSpans = exporter.getFinishedSpans();
    assert.strictEqual(finalSpans.length, 1006, "Should have 1006 spans total after cleanup test");
  });
});
